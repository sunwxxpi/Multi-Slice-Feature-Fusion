...
def trainer_coca(args, model, snapshot_path):
    ...
    if getattr(args, 'use_5fold_cv', False):
        # train = fold_idx 제외 4개 fold 합집합, val = fold_idx. per-case 볼륨에서 center 슬라이스 로드.
        ...
        for k in range(5):
            if k == args.fold_idx:
                continue
            train_samples.extend(_read_fold_list(args.list_dir_5fold, k))
    ...
    # Early stopping: val_loss 가 patience epoch 동안 min_delta 이상 개선 안 되면 중단 (patience=0 → 비활성)
    ...
    for epoch_num in tqdm(range(1, max_epoch + 1), ncols=70):
        ...
        # Early stopping: patience epoch 동안 best 미갱신 시 중단 (best_model.pth 는 이미 저장됨)
        if patience > 0 and patience_counter >= patience:
            logging.info(f"Early stopping at epoch {epoch_num} (no val_loss improvement for {patience} epochs, best={best_val_loss:.4f})")
            break
